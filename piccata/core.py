...
class _CoapMessageLayer(object):
    ...
    def _deduplicate_message(self, message):
        ...
        def _add_message_to_recent(cache, key):
            expiration_timer = Timer(EXCHANGE_LIFETIME, _remove_message_from_recent, (cache, key))
            expiration_timer.daemon = True
            expiration_timer.start()
            cache[key] = (message, expiration_timer)

        def _remove_message_from_recent(cache, key):
            try:
                cache.pop(key)
            except KeyError:
                logging.info("Failed to remove message from recent.")
